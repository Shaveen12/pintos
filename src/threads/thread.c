...
static struct list sleep_list; // global sleep list
...
bool thread_priority_comparator(struct list_elem *l, struct list_elem *s, void *aux UNUSED)
{
...
bool thread_donate_priority_comparator(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
...
void refresh_priority(void)
{
  ...
  {
    list_sort(&cur->donations, thread_donate_priority_comparator, 0);
...
void thread_unblock(struct thread *t)
{
  ...
  // thread_test_preemption(); //Causing timeout errors in the test, don't know why
}
...
void thread_yield(void)
{
  ...
  old_level = intr_disable();
  if (cur != idle_thread)
    list_insert_ordered(&ready_list, &cur->elem, thread_priority_comparator, 0);
...
void thread_sleep(int64_t wake_up_tick)
{
  ...
  // Disable interrupts while modifying thread state
  enum intr_level old_level = intr_disable();

  ASSERT(cur != idle_thread);

  cur->wake_up_tick = wake_up_tick; // store the wakeup time in the thread
